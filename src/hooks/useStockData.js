  ...
  useEffect(() => {
    ...
    api.stockcardByMidBrand(selectedCompany, selectedBranch.id, selectedBranch.code, brandid, dateRange.from, dateRange.to)
      .then(data => {
        ...
        // Group by type with subtotals for each type
        const grouped = [];
        const typeMap = {};
        let grandTotal = 0, grandValue = 0;
        
        // First pass: collect data by type
        for (const row of rawRows) {
          if (row.id === '-SUM') continue; // Skip grand total row
          
          const type = row.type || '(ไม่มี)';
          if (!typeMap[type]) {
            typeMap[type] = { rows: [], total: 0, value: 0 };
          }
          typeMap[type].rows.push(row);
          typeMap[type].total += row.total || 0;
          typeMap[type].value += row.value || 0;
          grandTotal += row.total || 0;
          grandValue += row.value || 0;
        }
