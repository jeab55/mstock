/**
 * ModalBrandPicker — brand ("ประเภท") picker filtered by selectedMtype
 * Title: 'Fsearch — เลือกประเภทสินค้า (brand)'
 * SQL: SELECT id, brandname FROM brand WHERE typeid = <selectedMtype> AND brandname LIKE '%xxx%' ORDER BY id
 */
...
const COLS = [
  ...
  { key: 'brandname', label: 'brandname', width: 340 },
];

export default function ModalBrandPicker({ onClose }) {
  const { selectedMtype, setSelectedBrand, selectedCompany } = useAppStore();
  ...
  // selectedMtype required; if null show message
  const noTypeid = !selectedMtype;

  // Load brands from brand table
  useEffect(() => {
    if (noTypeid) { setRows([]); return; }
    let cancelled = false;
    setLoading(true);
    api.brands(selectedCompany, selectedMtype, search)
      .then(data => {
        if (cancelled) return;
        setRows(data.rows || []);
      })
  ...
  const displayRows = [...rows, { id: '-SUM', brandname: 'สรุปยอด' }];
      ...
      <div className="bg-[#d4d0c8] border-2 shadow-lg flex flex-col" style={{ width: 460, height: 460, borderColor: '#808080' }}>
        ...
        <div className="flex items-center gap-1 px-2 py-1 flex-shrink-0" style={{ background: '#c0dcc0' }}>
          ...
          <input
            ...
            disabled={noTypeid}
          />
          <button className="delphi-btn px-2 py-0.5 text-xs" onClick={() => { setSelectedBrand(null); onClose(); }} disabled={noTypeid}>
            ทั้งหมด
          </button>
        </div>
          ...
          {noTypeid ? (
            ...
            <>
              ...
              {!loading && displayRows.map((row, ri) => {
                ...
                if ((isFocused || hovered === ri) && !isSUM) bg = '#e8f0fa';
                return (
                  <div key={ri}
        ...
        {/* Footer SQL hint + count */}
        <div className="h-10 flex flex-col gap-0.5 px-2 py-1 text-xs text-gray-600 border-t border-gray-400 flex-shrink-0 bg-gray-50" style={{ background: '#d4d0c8' }}>
          <div>SELECT id, brandname FROM brand WHERE typeid={selectedMtype} AND brandname LIKE '%{search}%'</div>
          <div>{noTypeid ? '—' : `${rows.length} รายการ`}</div>
        </div>
      </div>
    </div>
  );
