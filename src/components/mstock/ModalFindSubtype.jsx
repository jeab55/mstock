...
import { useMsubtypes } from '../../hooks/useStockData';

const COLS = [
  { key: 'id',   label: 'id',          width: 80 },
  { key: 'name', label: 'subtypename', width: 320 },
];

export default function ModalFindSubtype({ onClose }) {
  const { selectedMsubtype, setSelectedMsubtype } = useAppStore();
  const [search, setSearch] = useState('');
  const { rows: baseRows, loading } = useMsubtypes(search);
      ...
      <div className="bg-[#d4d0c8] border-2 shadow-lg flex flex-col" style={{ width: 460, height: 440, borderColor: '#808080' }}>
        ...
        <div className="flex items-center gap-1 px-2 py-1 flex-shrink-0" style={{ background: '#c0dcc0' }}>
          <input
            ...
            style={{ fontSize: '12px' }}
          />
          <button className="delphi-btn px-2 py-0.5 text-xs" onClick={() => { setSelectedMsubtype(null); onClose(); }}>
            ทั้งหมด
          </button>
        </div>
